function(task, responses){
  if(task.status.includes("error")){
      ...
      let entrySubTaskAction = function(data, entry) {
        if (entry["is_file"]) {
            // Always construct the path for the cat command
            let cat_parameters = "";
            if (entry["full_name"].startsWith("\\\\")) {
                // If the full_name is already a UNC path, use it directly
                cat_parameters = entry["full_name"];
            } else if (entry["full_name"].includes(":")) {
                // If the full_name contains a drive letter, use it as-is
                cat_parameters = entry["full_name"];
            } else if (data["host"] && data["host"].length > 0) {
                // If host is present, construct UNC path
                cat_parameters = `\\\\${data["host"]}\\${entry["full_name"]}`;
            } else {
                // Local file, just use the full_name
                cat_parameters = entry["full_name"];
            }
                      ...
                      value: [
                          ...
                          // },
                          {
                              ...
                              value: {
                                  ...
                                  rows: entry["permissions"]["acls"]?.map((permValue) => ({
                                      account: {
